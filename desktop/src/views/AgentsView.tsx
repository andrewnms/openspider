        ...
        <div className="grid md:grid-cols-2 gap-3">
          {agents.map((a) => (
            ...
            >
              ...
              {a.description && <div className="text-xs mt-1" style={{ color: 'var(--color-text-muted)' }}>{a.description.slice(0, 120)}</div>}
              <div className="text-[11px] mt-2 mono" style={{ color: 'var(--color-text-subtle)' }}>
                model: {a.model || '(none)'}
              </div>
            </button>
          ))}
          {agents.length === 0 && <div className="col-span-full text-sm py-6" style={{ color: 'var(--color-text-subtle)' }}>No agents yet. Click "New agent".</div>}
        </div>
      </div>
    </div>
  )
